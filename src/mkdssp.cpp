 ...
 * 
 */
void noTraj(string& input, ifstream& in, po::variables_map& vm);
void withTraj(string& input, ifstream& in, po::variables_map& vm,
              string dcdname);
...
// #endif
    ...
    if(withDCD)
      withTraj(input, infile, vm, dcdname);
    else
      noTraj(input, infile, vm);
...
//   exit(0);
  ...
  // delete protein and create it again at each dcd update ; not optimal so should optimise this later
  delete a;
  a = nullptr;
  in.close();
...
//   const vector<MChain*>& chains = a->GetChains();
  ...
  for(int i=0;i<dcdf.getNFILE();i++)
  {
    ...
    a = new MProtein();
    in.open(input,ios_base::in);
    
//     for(MChain* ch : a->GetChains())
//       delete ch;
//     cout << "chains cleaned" << endl;
...
//     cout << "from loop readpdb ok" << endl;
    
    // read one frame from a dcd file and get pointers to coordinates vectors
    dcdf.read_oneFrame();
    x=dcdf.getX();
    y=dcdf.getY();
    z=dcdf.getZ();
    ...
    for(MChain* chain : a->GetChains())
    {
//       vector<MResidue*>& residues = chain->GetResidues();
      for(MResidue* res : chain->GetResidues())
      {
        //reinitialise some parameters used for estimating secondary structure to default
...
/*
 * After parsing the pdb, when using the trajectory mode, this function is used for mapping the atom indices stored internally to the one from trajectory file
 */
vector<uint32> get_indices_of_atoms_to_update(MProtein& a)
{
